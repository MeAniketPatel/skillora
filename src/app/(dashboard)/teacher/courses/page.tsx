    ...
    <div className="space-y-6">
      ...
      {courses.length === 0 ? (
        <Card className="flex flex-col items-center justify-center p-12 text-center bg-white/70 dark:bg-neutral-900/70 border-dashed border-2 border-neutral-300 dark:border-neutral-700">
          ...
          <CardTitle className="text-xl font-bold">No courses created yet</CardTitle>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map((course) => (
            <Card key={course.id} className="overflow-hidden hover:shadow-lg transition-shadow bg-white dark:bg-neutral-900 border border-neutral-200/50 dark:border-neutral-800/50">
              <div className="aspect-video w-full bg-neutral-100 dark:bg-neutral-800 relative flex items-center justify-center">
                ...
                <div className={`absolute top-3 right-3 px-2 py-1 rounded text-xs font-semibold ${course.status === "PUBLISHED" ? "bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300" : "bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-300"}`}>
                  {course.status}
                </div>
              </div>
              <CardHeader className="p-4">
                <CardTitle className="text-lg line-clamp-1">{course.title}</CardTitle>
                <CardDescription className="text-xs">Level: {course.level}</CardDescription>
              </CardHeader>
